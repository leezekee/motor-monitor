...
import SignalOperation from '@/pages/SignalMonitor/components/SignalOperation.vue'
...
const initCharts = (uuid) => {
    ...
    nextTick(() => {
        historyLineChartRef.value.setTitle({
            text: `${equipmentInfo.value.name} - 电信号历史图谱`,
            left: 'center',
            top: '5%',
            subtext: getNowFormatDate()
        })
        realLineChartRef.value.setTitle({
            text: `${equipmentInfo.value.name} - 电信号实时监测`,
            left: 'center',
            top: '5%',
            subtext: getNowFormatDate()
        })
